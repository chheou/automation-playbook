...
def load_vault(master_pass: str) -> tuple[dict, bytes]:
    ...
    derived_key = b"\x00" * 32; derived_key = None   # wipe raw key

    try:
        vault_dict = json.loads(Fernet(fernet_key).decrypt(blob).decode("utf-8"))
    except Exception:
        hmac_key_bytes = b"\x00" * 32; hmac_key_bytes = None   # wipe on failure
...
def get_ssh_credentials() -> tuple[str, str]:
    ...
    master_pass = None
    hmac_key    = b"\x00" * 32; hmac_key = None   # not needed here
